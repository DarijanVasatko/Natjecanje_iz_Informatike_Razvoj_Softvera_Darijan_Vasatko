...
class ProizvodController extends Controller
{
    ...
    /**
     * Dohvati proizvode po ID-ovima (za Recently Viewed)
     */
    public function getByIds(Request $request)
    {
        ...
        // Ograniči na max 8 ID-ova
        $ids = array_slice($ids, 0, 8);

        // Dohvati proizvode i sortiraj po redoslijedu ID-ova
        $proizvodi = Proizvod::whereIn('Proizvod_ID', $ids)->get();

        // Sortiraj po originalnom redoslijedu
        $sorted = collect($ids)->map(function ($id) use ($proizvodi) {
            return $proizvodi->firstWhere('Proizvod_ID', $id);
        })->filter();

        return response()->json($sorted->map(function ($p) {
            return [
                'id' => $p->Proizvod_ID,
                'naziv' => $p->Naziv,
